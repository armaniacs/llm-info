...
2026-01-10 16:55 時点では、実装にはとりかからない。v2.0で追加する仕様とする。
...
--modelで指定したモデルが、複数のゲートウェイに存在する場合には、--gateway をつかって一意にするように促す。

--model を指定したときに、そのモデルの **API の実挙動から推定**する
...
テストデータ: 意味を持たない、あるいは一般的な長文（青空文庫や公文書など）に、特定の「検証用キーワード」を挿入したデータセットを作成する。
...
* **パスの定義:** 情報の埋め込み位置を固定することで、再現性を確保します。
...
1. **文末（Position: End）**
   - 最もシンプルな実装
   - 基本的なコンテキスト容量の測定に適している

2. **文の中央（Position: Middle）**
   - ロスト・イン・ザ・ミドル現象の観測
   - より厳密な測定が必要な場合に使用

3. **テキスト全体の80%位置（Position: 80%）**
   - 現実的な使用シナリオを想定
   - 実用的な限界値の発見に効果的

これらの位置は `--needle-position` オプションで指定可能とする（デフォルト: end）。


3. テストデータ生成の仕様

「大雑把に」とはいえ、手動でテキストを用意するのは現実的ではありません。以下のロジックでスクリプト（Go言語で実装）を用いて生成することを仕様に盛り込みます。
...
* 例：「[日付]のラッキーカラーは[色]です。」
* 埋め込み位置は `--needle-position` で指定されたルールに従う：
  - end: Bodyの末尾
  - middle: Bodyの中央
  - 80pct: 全体の80%位置
...
| `--model`      | Yes | 対象モデル ID                          |
| `--api-key`    | No  | APIキー                              |
| `--base-url`   | No  | Base URL                           |
| `--timeout`    | No  | HTTP タイムアウト（秒、default 30）         |
| `--max-trials` | No  | 試行回数上限（default 40）                |
| `--out`        | No  | 結果 JSON の出力先                      |
| `--verbose`    | No  | 詳細ログ出力                            |
| `--dry-run`    | No  | API 呼び出しせず探索計画のみ表示                |
| `--needle-position` | No | 検証用キーワードの埋め込み位置（end/middle/80pct, default: end） |
| `--format`     | No  | 出力形式（json/table, default: json）     |

#### 出力

* 出力形式: JSON または テーブル形式
* 標準出力および任意のファイル出力
* 各 trial の要約情報を含む（省略オプション可）
* テーブル形式の場合、人間が直接確認しやすい要約表示
