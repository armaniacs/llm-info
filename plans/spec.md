...
* **自動フォールバック**: まず `/v1/models` から基本情報を取得し、可能であれば `/model/info` から詳細情報を追加取得する。いずれかのエンドポイントが利用不可の場合でも、もう一方で情報取得を試みる。

(実装状況) 2026-01-10現在、OpenAI標準エンドポイント (`/v1/models`) を優先し、成功時にLiteLLM詳細情報の追加取得を試行します。標準エンドポイント失敗時はLiteLLMにフォールバックします。
