...
func TestClient_FetchModelsWithFallback(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			standardResponse:   `{"object": "list", "data": [{"id": "gpt-4", "object": "model", "created": 1234567890, "owned_by": "openai"}]}`,
			expectedModels:     []ModelInfo{{ID: "gpt-4", MaxTokens: 8192, Mode: "chat", InputCost: 0.00003}},
			wantErr:            false,
			expectFallback:     false,
		},
		{
			name:               "Standard succeeds, LiteLLM fails",
		...
		{
			name:               "Standard fails, LiteLLM succeeds",
			litellmStatusCode:  http.StatusOK,
			litellmResponse:    `{"models": [{"id": "claude-3", "max_tokens": 200000, "mode": "chat", "input_cost": 0.000015}]}`,
			standardStatusCode: http.StatusNotFound,
			standardResponse:   `{"error": "Not found"}`,
			expectedModels:     []ModelInfo{{ID: "claude-3", MaxTokens: 200000, Mode: "chat", InputCost: 0.000015}},
			wantErr:            false,
			expectFallback:     true,
		},
		{
