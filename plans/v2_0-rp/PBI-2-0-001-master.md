...
### v2.0 基本機能
- [x] 全てのBDDシナリオが自動テストでパス
- [x] テストカバレッジが90%以上
- [x] レビュー済みのコードがfeature/PBI-2-0-004-max-outputブランチにマージ済み
- [x] ユーザードキュメント（USAGE.md）更新済み
- [x] 実機でのE2Eテスト済み（OpenAI API使用）
- [x] CI/CDでの自動テスト実行済み
...
1. **PBI-2-0-002**: 技術調査 - OpenAI API基本連携 ✅ 完了 (2026-01-11)
2. **PBI-2-0-003**: Context Window推定アルゴリズム実装 ✅ 完了 (2026-01-11)
3. **PBI-2-0-004**: Max Output Tokens推定実装 ✅ 完了 (2026-01-11)
4. **PBI-2-0-005**: 出力形式とUX改善 ✅ 完了 (2026-01-11)
5. **PBI-2-0-006**: Needle In A Haystack機能 ✅ 完了 (2026-01-11)
6. **PBI-2-0-007**: パフォーマンス最適化とエラーハンドリング → **PBI-2-0-005**: APIタイムアウト問題修正 ✅ 完了 (2026-01-11)

## 将来の拡張（v2.1以降）

### v2.1 実装済み機能
- **PBI-2-1-001**: ログと結果保存機能 ✅ 完了
- **PBI-2-1-002**: 統合probeコマンド（同時実行とマージ） ✅ 完了 (2026-01-12)
